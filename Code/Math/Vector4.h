...
public:
   ...
   inline Vector4(Number x, Number y, Number z = 0, Number w = 1) : x(x), y(y), z(z), w(w)
   {
   }

   inline Vector4(const Vec2& v, Number z = 0, Number w = 1) : x(v.x), y(v.y), z(z), w(w)
   {
   }

   inline Vector4(const Vec3& v, Number w = 1) : x(v.x), y(v.y), z(v.z), w(w)
   {
   }
...
template <typename T> const Vector4<T> Vector4<T>::Forward(0.0f, 1.0f, 0.0f);
template <typename T> const Vector4<T> Vector4<T>::Up(0.0f, 0.0f, 1.0f);
#endif //__RIGHT_HANDED__

#ifdef __LEFT_HANDED__
template <typename T> const Vector4<T> Vector4<T>::Right(1.0f, 0.0f, 0.0f);
template <typename T> const Vector4<T> Vector4<T>::Up(0.0f, 1.0f, 0.0f);
template <typename T> const Vector4<T> Vector4<T>::Forward(0.0f, 0.0f, 1.0f);
