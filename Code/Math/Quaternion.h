...
public:
  ...
  inline Quaternion(float x, float y, float z) : Quaternion(nullptr)
  {
  ...
  {
    AssertMessage(mat.IsOrthogonal(), "[Quaternion] Given matrix is not orthogonal!", mat);
    AssertMessage(IsEqual(mat.Determinant(), 1.0f), "[Quaternion] Matrix Determinant should be 1, but ", mat.Determinant());
  ...
  {
    AssertMessage(from.IsUnity(), "Quaternion slerp should have unit length",
      ", but ", from.vector.Length());
  ...
  {
    AssertMessage(unitAxis.IsUnity()
      , "Quaternion Length = ", unitAxis.Length());
